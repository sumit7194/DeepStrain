...
- **False-alarm rate pushed to 1/decade on an 80-year background.** Global time-slides over 100 O4b coincident segments (114 h) give **6,199 distinct lags → 80.5 years** of accidental-coincidence livetime (17× the previous best) — background grows as N_segments², so this ran on a laptop. Thresholds: 1/month 12.34, 1/year 14.11, **1/decade 16.12**. No candidate in 114 h of O4b.
- **…then audited until it gave up its real error bar.** The detector-independence assumption time-slides rest on genuinely holds (zero-lag correlation *p* = 0.69). But 80.5 years is *livetime*, not 80.5 years of independent statistics: the 8 background events setting 1/decade trace back to **just 2 distinct Hanford windows**, and 6 of the 8 loudest windows in all 114 h sit in **one segment** — dropping it alone moves the threshold 16.12 → 11.26. A leave-block-out jackknife gives **±33–44%**, where the Poisson band had implied ±2%. **The null got stronger in the process:** the "8% near-miss" (11.295) turns out to be a one-sided Hanford glitch (H1 +12.5, L1 −1.2) with Livingston seeing nothing, and the search is null in **4/4** configurations at margins of **2.0–3.7×**. A consistency statistic halves the background instability but buys **no reach** (0.97×) — an honest negative that extends the earlier "sum is optimal" finding into the deep regime.
...
| Subsolar deep-FAR audit | thresholds **±33–44%** (one glitchy segment), not ±2%; null **4/4** at 2.0–3.7× margin | ✓ gated |
...
verify.sh                      regression gate over every headline artifact (46 checks)
...
./verify.sh                              # from the root: 46 checks assert every headline result
```
